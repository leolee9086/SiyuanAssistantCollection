...
async function 执行任务(renderInstance, 编辑器上下文,编辑器上下文特征向量) {
    ...
    try {
        ...
        if (编辑器上下文特征向量 && renderInstance.renderEditorVectorTips) {
            console.error(编辑器上下文特征向量)
            sac.logger.tipsLog(`触发编辑器上下文向量索引,正在生成tips`)
            let data = await renderInstance.renderEditorVectorTips(编辑器上下文);
            if (data) {
                data.source = renderInstance.name;
                $处理并显示tips(data);
            }
        }else{
            console.error(renderInstance.renderEditorVectorTips)
        }
    } catch (e) {
        console.warn(e);
    }
