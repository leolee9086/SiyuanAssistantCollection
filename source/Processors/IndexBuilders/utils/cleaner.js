...
export function 定时实行块索引添加(retryInterval = 1000) {
    if (待索引数组.length > 0) {
        ...
        for (let i = 0; i < workerCount; i++) {
            ...
            let 待处理的块 = 待索引数组.splice(batchStartIndex, 每个批次的块数量);
            if (待处理的块.length > 0) {
                let 索引开始时间 = Date.now(); // 记录索引开始的时间
                
                indexBlocks(待处理的块, (结果数组) => {
                    let 索引结束时间 = Date.now(); // 记录索引结束的时间
                    let 索引耗时 = 索引结束时间 - 索引开始时间; // 计算索引耗时

                    // 假设结果数组与待处理的块一一对应
                    结果数组.forEach((结果, index) => {
                        if (!(结果 && 结果.data && 结果.data.length > 0)) {
                            // 当返回值中对应下标的结果的data不是一个非空数组时，将块放回待索引数组
                            待索引数组.splice(batchStartIndex + index, 0, 待处理的块[index]);
                        } else {
                            添加到入库队列(
                                {
                                    id: 待处理的块[index].id,
                                    meta: {
                                        link: `siyuan://blocks/${待处理的块[index].id}`,
                                        box: 待处理的块[index].box,
                                        hash: 待处理的块[index].hash
                                    },
                                    vector: {
                    ...
                    if(结果数组.length){
                        console.log(`已索引以下${待处理的块.length}个块: \n${待处理的块.map(块 => 块.id)};\n索引耗时:${索引耗时}毫秒,待索引块数量为${待索引数组.length}个;本轮平均处理时长为${Math.floor(索引耗时 / 待处理的块.length)}毫秒`);
