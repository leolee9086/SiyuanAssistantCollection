...
export const rssPackages =class 
{
    //包类型的名称
    constructor(){
       this.name= 'sac-rss-adapter'
    }
    //包类型的元数据名
    meta= 'rss.json'
    //包类型的设置文件名称,可以为空
    ...
    load= async (packageName, fileName) => {
        console.log(rssPackages)
        //这里由加载器注入
        return await lazyloadRouteHandler(this.local.resolve(packageName, fileName))
    }
    descriptions= {
